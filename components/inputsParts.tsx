...
export const inputSizeVariant: SizeVariantSpreadFunction<any> = (
  ...
  const radiusToken = extras.tokens.radius[val as keyof typeof extras.tokens.radius] ?? extras.tokens.radius['true']
...
const InputIcon = InputIconFrame.styleable<{
  ...
  const color = getVariable(
    contextColor || theme[contextColor as keyof typeof theme]?.get('web') || theme.color10?.get('web')
  )
...
export const InputContainerFrame = styled(View, {
  ...
  variants: {
    size: {
      '...size': (val, { tokens }) => ({
        gap: (tokens.space[val as keyof typeof tokens.space] as any).val * 0.3,
      }),
    },
    color: {
      '...color': () => ({}),
...
export const InputInfo = styled(Text, {
  ...
  variants: {
    size: {
      '...fontSize': (val, { font }) => {
        if (!font) return
        const fontSize = (font.size[val as keyof typeof font.size]?.val ?? 0) * 0.8
  ...
  variants: {
    size: {
      '...size': (val: SizeTokens, { tokens }) => {
        const radiusToken = tokens.radius[val as keyof typeof tokens.radius] ?? tokens.radius['true']
        return {
